...
export default function Sidebar({ selectedChatId, onChatSelect, isOpen, onClose }: SidebarProps) {
  // На десктопе сайдбар всегда виден (isOpen игнорируется), на мобильных - управляется через isOpen
  // Если isOpen не передан, считаем что это десктоп и сайдбар всегда виден
  const isOpenOnMobile = isOpen !== undefined ? isOpen : true
  ...
  return (
    <>
      ...
      {onClose && (
        <div className="p-4 border-b border-[#3A3A3A] flex items-center justify-between md:hidden">
          <button
            onClick={onClose}
            className="p-2 text-[#EDEDED] hover:text-[#E49E00] transition-colors"
