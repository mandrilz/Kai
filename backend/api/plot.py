...
async def plot_compare(
    ...
    competitor_articul: Optional[str] = Query(None, description="Артикул насоса конкурента (устаревший, используйте competitor_model)"),
    competitor_model: Optional[str] = Query(None, description="Модель насоса конкурента для сравнения (например: CDM 3-2, Grundfos CM)"),
    ...
    **Параметры:**
    ...
    - `competitor_model`: Модель насоса конкурента для сравнения (опционально, например: "CDM 3-2", "Grundfos CM", "Lowara e-SV")
    - `competitor_articul`: Артикул насоса конкурента (устаревший, используйте competitor_model)
    ...
    **Примеры:**
    ...
    - `/api/plot/compare?kometta_articul=13214201&competitor_model=CDM%203-2`
    - `/api/plot/compare?kometta_articul=13214201&competitor_model=Grundfos%20CM`
    - `/api/plot/compare?kometta_articul=13214201&q_min=0&q_max=200`
    """
    try:
        # Определяем модель конкурента: приоритет у competitor_model, затем competitor_articul (для обратной совместимости)
        competitor_query = competitor_model or competitor_articul
        
        # Создаем объект запроса для валидации
        request = PlotRequest(
            kometta_articul=kometta_articul,
            competitor_model=competitor_query,
            q_min=q_min,
            q_max=q_max
        )
        ...
        # Определяем диапазон Q с учетом всех серий (Кометта + конкурент)
        all_q_points = list(q_points_kometta) if q_points_kometta else []
        all_h_points = list(h_points_kometta) if h_points_kometta else []
        
        if competitor_data and competitor_data.get("q_points"):
            all_q_points.extend(competitor_data["q_points"])
            all_h_points.extend(competitor_data.get("h_points", []))
        
        # Вычисляем bounds по всем точкам
        q_min_data = min(all_q_points) if all_q_points else 0
        q_max_data = max(all_q_points) if all_q_points else 100
        h_min_data = min(all_h_points) if all_h_points else 0
        h_max_data = max(all_h_points) if all_h_points else 100
        
        # Применяем пользовательские ограничения или используем данные
        q_min = request.q_min if request.q_min is not None else q_min_data
        q_max = request.q_max if request.q_max is not None else q_max_data
        
        # Добавляем padding: ±5-10% диапазона или минимум 1-2 единицы
        q_range = q_max - q_min
        q_padding = max(q_range * 0.1, 2.0) if q_range > 0 else 2.0
        q_min_plot = max(0, q_min - q_padding)
        q_max_plot = q_max + q_padding
        
        q_plot = np.linspace(q_min_plot, q_max_plot, 100)
        ...
        # Вычисляем максимальный H из всех серий для правильного масштабирования
        max_h_plot = max(h_plot_kometta) if h_plot_kometta else h_max_data
        if competitor_data and h_plot_comp:
            max_h_plot = max(max_h_plot, max(h_plot_comp))
        
        # Добавляем padding для оси Y: ±5-10% диапазона или минимум 1-2 м
        h_range = max_h_plot - h_min_data
        h_padding = max(h_range * 0.1, 2.0) if h_range > 0 else 2.0
        h_min_plot = max(0, h_min_data - h_padding)
        h_max_plot = max_h_plot + h_padding
        ...
        ax.set_ylim(h_min_plot, h_max_plot)
        ax.axhline(y=0, color='black', linewidth=2)
        ax.axvline(x=0, color='black', linewidth=2)
