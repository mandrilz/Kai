...
def generate_komettik_response(
    ...
    try:
        ...
        import os
        from datetime import datetime
        log_path = os.getenv("DEBUG_LOG_PATH", "c:\\Users\\Admin\\Desktop\\3\\.cursor\\debug.log")
        try:
        ...
        if is_q:
            ...
            # Если это вопрос и (нет TECH_QA ответа или нужен fallback) - ищем в KB
            # TECH_QA уже использует KB, но если ответ короткий/неполный - дополняем
