...
import asyncio
import random
from services.intents import detect_intent, Intent
...
def generate_komettik_response(intent: str, data: Dict[str, Any], message: str, attachments: List[Attachment]) -> str:
    """
    ...
    if intent == Intent.OFF_TOPIC:
        ...
        if data.get("action") == "analog_by_articul":
            ...
            if articul:
                ...
                response_parts.append("Укажите артикул насоса Кометта.")
        # Проверяем, это запрос по артикулу?
        elif data.get("action") == "by_articul":
            articul = data.get("articul", "")
            if articul:
                from services.selector import get_pump_curve_data
                pump_data = get_pump_curve_data(articul)
    ...
    # Уточняющие вопросы уже добавлены в блоках SELECTION_BY_POINT, ANALOG_BY_MODEL, DIAGNOSTICS
    if intent not in [Intent.OFF_TOPIC, Intent.SELECTION_BY_POINT, Intent.ANALOG_BY_MODEL, Intent.DIAGNOSTICS]:
        response_parts.append("\n\nЧто ещё могу помочь?")
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        # Определяем интент
        has_attachments = len(request.attachments) > 0
        intent_result = detect_intent(request.message, has_attachments)
