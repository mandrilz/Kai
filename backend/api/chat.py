        ...
        if intent == Intent.OFF_TOPIC:
            ...
            if not q or not h:
                ...
                if q and h:
                    ...
                    update_state(session_id, {"flow_m3h": q, "head_m": h}, keep_pending_if_not_set=True)
                ...
                if not is_valid:
                    ...
                    try:
                        ...
                        pump_type_info = data.get("pump_type_info") or {}
                        series_filter = pump_type_info.get("series") if isinstance(pump_type_info, dict) else None
                        top_n = 200 if (series_filter or body_material_code) else 50
                        pumps = select_by_working_point(q, h, h_st=h_st, top_n=top_n)

                        # Фильтрация по серии Кометта (К377/К144/...)
                        if series_filter:
                            def _norm(s: str) -> str:
                                return (s or "").upper().replace("K", "К")

                            series_norm = [_norm(s) for s in (series_filter or [])]
                            ...
                            # Если нашли по типу — показываем только их (как вы просили)
                            if filtered:
                                pumps = filtered

                            # Предпочтение складской программы для К144: модели с "/04А/" (или "/04A/")
                            if any(sn == "К144" for sn in series_norm):
                                import re

                                def _is_stock_pref(p: Dict[str, Any]) -> int:
                                    m = str(p.get("model", "") or "")
                                    # ищем /04А/ или /04A/
                                    return 0 if re.search(r"/0*4[АA]/", m, re.IGNORECASE) else 1
                        ...
                        log_error(e, "chat.py:generate_komettik_response:select_by_working_point", 
                                 context={"q": q, "h": h, "h_st": h_st}, session_id=session_id)
                        response_parts.append(
            ...
            try:
                ...
                if new_model_in_message and pending_confirmation:
                    ...
                    update_state(session_id, {"pending_analog_confirmation": confirmation_data})
                ...
                if pending_confirmation:
                    ...
                    if is_positive:
                        ...
                        update_state(session_id, {"pending_analog_confirmation": None})
                ...
                log_error(e, "chat.py:generate_komettik_response:ANALOG_BY_MODEL", 
                         context={"intent": intent, "data": data}, session_id=session_id)
                response_parts.append(
            ...
            try:
                ...
                log_error(e, "chat.py:generate_komettik_response:FILE_UPLOAD", 
                         context={"intent": intent, "attachments_count": len(attachments)}, session_id=session_id)
                response_parts.append(
            ...
            try:
                ...
                log_error(e, "chat.py:generate_komettik_response:TECH_QA", 
                         context={"intent": intent, "query": query}, session_id=session_id)
                response_parts.append(
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        try:
            pending_confirmation = get_context_for_model(request.session_id).get("state", {}).get("pending_analog_confirmation")
        except Exception:
        ...
        session_id = request.session_id if 'request' in locals() else "unknown"
        error_message = handle_error(
            e,
