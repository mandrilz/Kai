            ...
            try:
                ...
                if not q or not h:
                    ...
                    if not is_valid:
                        ...
                        # Инициализируем pump_type_info по умолчанию
                        pump_type_info = {}
                        try:
                ...
                # Обработка ошибок при подборе по рабочей точке
                from services.error_handler import handle_error, log_error
                log_error(e, "chat.py:generate_komettik_response:SELECTION_BY_POINT", 
                         context={"intent": intent, "data": data, "q": q if 'q' in locals() else None, "h": h if 'h' in locals() else None}, 
                         session_id=chat_id)
                response_parts.append(
