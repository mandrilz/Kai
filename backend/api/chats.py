...
FOLLOWUP_QUESTIONS = [
    "Какая у вас рабочая точка: подача Q (м³/ч) и напор H (м)?",
    "Какая жидкость и температура перекачиваемой среды?",
    "Хотите, я построю график Q‑H и (при необходимости) сравню с конкурентом?",
]


def ensure_dialogue_continuation(text: str) -> str:
    """
    Server-side guard: в конце КАЖДОГО ответа добавляем 1–3 продолжения диалога.
    """
    t = (text or "").strip()
    if not t:
        t = "Я Кометтик — инженер‑помощник Кометта."

    # Если в конце уже есть вопросы/предложения — не плодим дубль.
    tail = t[-400:]
    if "?" in tail:
        return t

    # Добавляем 2 коротких вопроса (MVP). Можно расширить под контекст.
    return t + "\n\n" + "\n".join([
        f"— {FOLLOWUP_QUESTIONS[0]}",
        f"— {FOLLOWUP_QUESTIONS[1]}",
    ])
...
async def send_message(
    ...
    try:
        ...
        try:
            ...
            response_text = ensure_dialogue_continuation(response_text)
        except Exception as e:
