...
class MessageResponse(BaseModel):
    ...
    printed_at: Optional[datetime] = None  # Время окончания печати эффектом
    created_at: datetime
...
class ChatMessageRequest(BaseModel):
    ...
    idempotency_key: Optional[str] = None  # Ключ для предотвращения дубликатов
...
async def send_message(
    ...
    from services.logging_utils import generate_correlation_id, log_request, log_response, estimate_token_count
    import time
    ...
    try:
        ...
        # Проверяем idempotency key для предотвращения дубликатов
        idempotency_key = message_data.idempotency_key
        if idempotency_key:
            # Ищем существующее сообщение с таким же idempotency_key в этом чате
            ...
            if existing_message:
                # Найдено дубликат - ищем соответствующий ответ ассистента
                existing_response = session.exec(
                    select(Message)
                    .where(Message.chat_id == chat_id)
                    .where(Message.role == "assistant")
                    .where(Message.created_at > existing_message.created_at)
                    .order_by(Message.created_at.desc())
                ).first()
                
                if existing_response:
                    # Возвращаем существующий ответ через SSE
                    # stream_response уже импортирован в начале файла
                    return StreamingResponse(
                        stream_response(existing_response.content, correlation_id=correlation_id),
        ...
        # Сериализуем attachments в JSON строку, если есть
        attachments_json = None
        if message_data.attachments and len(message_data.attachments) > 0:
            attachments_json = json_module.dumps(message_data.attachments)
        ...
        user_message = Message(
            ...
            idempotency_key=idempotency_key,  # Сохраняем idempotency_key для проверки дубликатов
        )
        ...
        # Оцениваем количество токенов в ответе
        ...
        # Оцениваем токены контекста для логирования
        context_token_estimate = estimate_token_count(str(context_for_intent))
        ...
        log_response(
            ...
            summary_used=context_for_intent.get("summary_used", False),
            recent_messages_count=len(context_for_intent.get("recent_messages", [])),
            pending_slot=context_for_intent.get("pending_slot"),
            # Не логируем полный текст пользователя (PII), только длину и хэш
            user_message_length=len(normalized_message),
            user_message_hash=hash(normalized_message) % (10**9)  # Хэш для идентификации без PII
        )
        
        # Возвращаем потоковый ответ с correlation_id в заголовках
        return StreamingResponse(
            stream_response(response_text, correlation_id=correlation_id),
            media_type="text/event-stream",
            headers={
                "Cache-Control": "no-cache",
                "Connection": "keep-alive",
                "X-Correlation-ID": correlation_id,  # Добавляем correlation_id в заголовки
            }
        )
        ...
        logger.error(f"[{correlation_id if 'correlation_id' in locals() else 'unknown'}] Error in send_message: {str(e)}\n{error_trace}")
...
@router.post("/{chat_id}/messages/{message_id}/printed")
async def mark_message_printed(
    chat_id: UUID,
    message_id: UUID,
    request: Request,
    session: Session = Depends(get_db_session),
    ...
    """
    Отмечает сообщение как напечатанное (завершает эффект печати).
    
    Используется фронтендом после завершения эффекта печати для assistant сообщений.
    После F5 сообщения с printed_at показываются сразу, без эффекта печати.
    """
    user_id = await require_auth(request)
    chat = verify_chat_ownership(chat_id, user_id, session)
    ...
    # Отмечаем как напечатанное (только для assistant сообщений)
    if message.role == "assistant" and not message.printed_at:
        message.printed_at = datetime.utcnow()
        session.add(message)
        session.commit()
    
    return {"status": "ok", "message_id": str(message_id), "printed_at": message.printed_at.isoformat() if message.printed_at else None}
