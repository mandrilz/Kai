...
- Поле `printed_at` используется для персистентного хранения флага "напечатано" для assistant сообщений, чтобы эффект печати не запускался повторно после F5