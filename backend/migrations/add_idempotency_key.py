"""
Миграция: добавление поля idempotency_key в таблицу message.
...
def migrate():
    """Добавляет поле idempotency_key в таблицу message."""
    ...
    try:
        ...
        if "idempotency_key" in columns:
            print("✅ Колонка idempotency_key уже существует")
        else:
            # Добавляем колонку
            cursor.execute("ALTER TABLE message ADD COLUMN idempotency_key TEXT")
            print("✅ Колонка idempotency_key добавлена")
        
        # Проверяем, существует ли индекс
        cursor.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_message_idempotency_key'")
        if cursor.fetchone():
            print("✅ Индекс idx_message_idempotency_key уже существует")
        else:
            # Создаем индекс
            cursor.execute("CREATE INDEX idx_message_idempotency_key ON message(idempotency_key)")
            print("✅ Индекс idx_message_idempotency_key создан")
...
if __name__ == "__main__":
    print(f"Миграция: добавление idempotency_key в таблицу message")
