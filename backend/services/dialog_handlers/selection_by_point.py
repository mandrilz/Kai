    ...
    *,
    ...
    try:
        ...
        for i, pump in enumerate(pumps, 1):
            ...
            # Ссылка на лист данных только если есть валидный URL
        ...
        # Предлагаем уточнить параметры жидкости только если они еще не указаны
        # Проверяем, заполнена ли жидкость в состоянии
        fluid = (context.get("state") or {}).get("fluid")
        if not fluid:
            # Жидкость не указана - предлагаем уточнить (но не добавляем прямой вопрос, 
            # так как это делает slot_extractor через pending_slot)
            res.response_parts.extend(get_selection_fluid_params())
        
        # Добавляем общие вопросы и предложения
