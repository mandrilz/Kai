...
def parse_short_answer(text: str, pending_slot: Optional[str], chat_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    # Обработка ответов "не знаю", "неважно" и т.д. - пропускаем слот без уточнения
    skip_phrases = [
        "не знаю", "незнаю", "не знаю", "не знаю точно", "не знаю точно",
        "неважно", "не важно", "не важно", "не имеет значения", "не имеет значения",
        "безразлично", "без разницы", "безразницы", "всё равно", "все равно",
        "не имеет значения", "несущественно", "не существенно",
        "пропустить", "пропусти", "можно пропустить", "можно не указывать",
        "не указано", "не указан", "не указана", "не указаны",
        "не знаю точно", "не уверен", "не уверена", "не уверен точно",
        "не помню", "не помню точно", "забыл", "забыла",
    ]
    
    if any(phrase in text_lower for phrase in skip_phrases):
        # Для концентрации, если жидкость - вода, устанавливаем 0
        if pending_slot == "concentration" and chat_id:
            # Получаем состояние через get_conversation, так как context не передан
            try:
                ...
                # Если не удалось получить состояние, просто пропускаем слот
                pass
        ...
        return {
            ...
            "confidence": 0.5,
            "next_slot": None,  # Переходим к следующему слоту (определится автоматически)
            "skip_slot": True,  # Флаг для пропуска слота
