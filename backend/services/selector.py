...
def select_by_working_point(q: float, h: float, top_n: int = 3) -> List[Dict[str, Any]]:
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            # Фильтруем мусор (слишком большая ошибка или отрицательный напор)
            if h_work < 0 or error > 2.0:  # Ошибка не более 2.0 (200% относительной ошибки)
                continue
...
def find_analog_by_model(competitor_model: str, top_n: int = 3) -> Tuple[Optional[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    a_comp, b_comp, c_comp, d_comp = approximate_curve(q_comp, h_comp)
    ...
    for idx, row in df_kometta.iterrows():
        ...
        try:
            a_kometta, b_kometta, c_kometta, d_kometta = approximate_curve(q_kometta, h_kometta)
