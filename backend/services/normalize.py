...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Список колонок, которые НЕ должны преобразовываться в float
    protected_columns = ["brand", "model", "series", "articul", "url", "datasheet_url"]
    
    # Нормализуем числовые колонки (graphic_*)
    for col in df_normalized.columns:
        if col.startswith("graphic_") or col in ["power", "rpm", "voltage"]:
            df_normalized[col] = df_normalized[col].apply(normalize_float)
        elif col not in protected_columns and df_normalized[col].dtype == "object":
            # Пробуем преобразовать строковые числовые колонки (но не модель/бренд)
            try:
                df_normalized[col] = df_normalized[col].apply(normalize_float)
            except:
                pass
