...
def normalize_text(text: str) -> str:
    ...
    # ИСПРАВЛЕНО: Убираем пробелы в единицах измерения ДО замены кириллицы
    # Это важно для распознавания "140 м3 / час" → "140 м3/час"
    text = re.sub(r'м\s*3\s*/\s*ч', 'м3/ч', text)
    text = re.sub(r'м\s*3\s*/\s*час', 'м3/час', text)
    text = re.sub(r'м\s*3\s*/\s*с', 'м3/с', text)
    text = re.sub(r'м\s*3\s*/\s*сек', 'м3/сек', text)
    text = re.sub(r'м\s*3\s*/\s*мин', 'м3/мин', text)
    text = re.sub(r'л\s*/\s*с', 'л/с', text)
    text = re.sub(r'л\s*/\s*сек', 'л/сек', text)
    text = re.sub(r'л\s*/\s*мин', 'л/мин', text)
    text = re.sub(r'л\s*/\s*ч', 'л/ч', text)
    text = re.sub(r'л\s*/\s*час', 'л/час', text)
