...
def normalize_text(text: str) -> str:
    ...
    # Также обрабатываем варианты: м3 / час, м 3 / час, м3/ час и т.д.
    text = re.sub(r'м\s*3\s*/\s*ч(?:ас)?', 'm3/h', text)
    text = re.sub(r'm\s*3\s*/\s*h(?:our|ours)?', 'm3/h', text, flags=re.IGNORECASE)
    # Обрабатываем вариант "м3 / час" с пробелами вокруг "/"
    text = re.sub(r'м\s*3\s*/\s*час', 'm3/h', text)
