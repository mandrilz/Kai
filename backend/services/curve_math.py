...
def get_q_for_h(h_val: float, a: float, b: float, c: float, d: float, q_min: float = 0.0, q_max: float = 10000.0) -> float:
    ...
    ИСПРАВЛЕНО: Теперь ищет максимальный напор по всей кривой, а не только в q_min.
    ...
    try:
        # ИСПРАВЛЕНО: Находим максимальный напор на всей кривой, а не только в q_min
        # Проверяем несколько точек для определения максимального напора (shut-off head)
        # Обычно максимальный напор при Q=0 или близко к 0
        test_points = [0.0, q_min, (q_min + q_max) / 2, q_max]
        h_max = max([calculate_h(q, a, b, c, d) for q in test_points if q >= 0])
        
        # Если требуемый напор выше максимального, решения нет
        if h_val > h_max:
            return 0.0
        
        # Ищем корень на всем диапазоне [0, q_max]
        # Используем расширенный диапазон для поиска
        search_min = max(0.0, q_min * 0.5)  # Немного расширяем влево
        search_max = min(q_max, q_max * 1.1)  # Немного расширяем вправо
        
        sol = root_scalar(func, bracket=(search_min, search_max), method='brentq')
        if sol.converged:
            q_result = float(sol.root)
            # Проверяем, что результат в разумных пределах
            if 0 <= q_result <= q_max:
                return q_result
    except ValueError:
        # root_scalar может выбросить ValueError если нет корня в диапазоне
        pass
    except Exception:
        # Другие ошибки (например, если кривая некорректна)
        pass
    return 0.0
