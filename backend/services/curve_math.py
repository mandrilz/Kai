...
def approximate_curve(q_points: List[float], h_points: List[float]) -> Optional[Tuple[float, float, float, float]]:
    ...
    на основе точек (Q, H) методом least-squares.
    
    ОБНОВЛЕНО: Теперь работает с N>=4 точками через numpy.polyfit (least-squares fit).
    Если точек ровно 4 - использует точное решение через np.linalg.solve (как раньше).
    Если точек больше 4 - использует least-squares fit для лучшей аппроксимации.
    ...
    if len(q_points) != len(h_points):
        return None
    
    Q = np.array(q_points)
    H = np.array(h_points)
    
    # Если ровно 4 точки - используем точное решение (как раньше)
    if len(q_points) == 4:
    ...
    # Если точек больше 4 - используем least-squares fit через numpy.polyfit
    try:
        # numpy.polyfit возвращает коэффициенты в порядке [d, c, b, a] (от младшей степени к старшей)
        # Нам нужен порядок [a, b, c, d] (от старшей к младшей)
        coeffs = np.polyfit(Q, H, deg=3)
        # Переворачиваем: [d, c, b, a] -> [a, b, c, d]
        return float(coeffs[3]), float(coeffs[2]), float(coeffs[1]), float(coeffs[0])
