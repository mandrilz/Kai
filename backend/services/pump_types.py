...
def generate_type_response(type_info: Dict[str, Any]) -> str:
    ...
    Args:
        type_info: результат detect_pump_type()
    ...
    if type_info["availability"] == PumpTypeAvailability.AVAILABLE:
        # Тип есть у Кометта
        series_list = ", ".join(type_info["series"])
        ...
        response_parts = [
            ...
            f"\n\n**{type_info['type_name']}** — серия **{series_list}**",
        ...
        # Типа нет у Кометта
        intro_variants = [
            f"Понял, вы ищете {type_info['type_name'].lower()}.",
            f"Вижу, нужен {type_info['type_name'].lower()}.",
        ]
        ...
        if type_info.get("alternative_suggestion"):
            response_parts.append(f"\n\n**Вариант 2:** {type_info['alternative_suggestion']}")
        
        response_parts.append("\n\n**Вариант 3:** Если критична именно конструкция, уточните параметры — подскажу, что из Кометта максимально близко.")
        
        # Уточняющие вопросы
        if type_info.get("clarifying_questions"):
            response_parts.append("\n\n**Уточняющие вопросы:**")
            for i, q in enumerate(type_info["clarifying_questions"][:3], 1):
                response_parts.append(f"\n{i}. {q}")
