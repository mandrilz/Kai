...
class Message(SQLModel, table=True):
    ...
    idempotency_key: Optional[str] = Field(default=None, index=True)  # Ключ для предотвращения дубликатов
    printed_at: Optional[datetime] = Field(default=None, index=True)  # Время окончания печати эффектом (для assistant сообщений)
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class ConversationState(SQLModel, table=True):
    ...
    last_summarized_message_count: Optional[int] = Field(default=0)  # Количество сообщений при последнем суммировании
    updated_at: datetime = Field(default_factory=datetime.utcnow)
